...
// With a value
console.log(`${nullishExpr} (value=5, default=10) = ${
  evaluate(nullishAst, { data: { value: 5, defaultVal: 10 }, nodes: nullishNodes })
}`);

// With arithmetic
const nullishExpr2 = "value ?? 10 + 5";
const [nullishAst2] = nullishParser.parse(nullishExpr2, { value: "number" });
console.log(`${nullishExpr2} (value=3) = ${
  evaluate(nullishAst2, { data: { value: 3 }, nodes: nullishNodes })
}`);
...
console.log(`${unitExpr1} = ${(evaluate(unitAst1, { data: {}, nodes: unitNodes }) as number).toFixed(2)}`);
...
console.log(`${unitExpr3} = ${(evaluate(unitAst3, { data: {}, nodes: unitNodes }) as number).toFixed(2)}`);
